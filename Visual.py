...
def plot_data(CPvGTI_obj, basis = "umap", figsize = (15,7), save_as = None, title = None, **kwargs):
    """\
    Description
...
#
    X = CPvGTI_obj.adata.obsm["X_" + basis]
    ...
    if "clusters" in CPvGTI_obj.adata.obs.columns:
        cluster_anno = [x for x in CPvGTI_obj.adata.obs["clusters"].values]
        cluster_uni = [x for x in np.unique(CPvGTI_obj.adata.obs["clusters"].values)]
    ...
    elif "sim_time" in CPvGTI_obj.adata.obs.columns:
        X_ordered = X[np.argsort(CPvGTI_obj.adata.obs["sim_time"].values),:]
        ...
        pic = ax.scatter(X_ordered[:,0], X_ordered[:,1], cmap = "inferno", c = np.arange(X_ordered.shape[0]), alpha = 1, s = _kwargs["s"])
...
def first_order_approx_pt(CPvGTI_obj, basis = "pca", trajs = 4, figsize= (20,20), save_as = None, title = None, axis = True):
    """\
    Description        
    ...
    adata = CPvGTI_obj.adata
    basis = 'X_' + basis
    if basis not in adata.obsm.keys():
        raise ValueError("basis incorrect")


    for i in range(min(trajs, CPvGTI_obj.pseudo_order.shape[1])):
        sorted_pt = CPvGTI_obj.pseudo_order["traj_"+str(i)].dropna(axis = 0).sort_values()
        ...
        if nrows != 1:
            ...
            axs[i%nrows, i//nrows].scatter(adata.obsm[basis][:,0],adata.obsm[basis][:,1], color = 'black', alpha = 0.1)
            ...
            axs[i%nrows, i//nrows].set_title("CPvGTI: Path " + str(i), fontsize = 25)
            ...
            if not axis:
                axs.axis("off")            
            axs.scatter(adata.obsm[basis][:,0],adata.obsm[basis][:,1], color = 'black', alpha = 0.1)

            pseudo_visual = axs.scatter(X_traj[:,0],X_traj[:,1],c = np.arange(X_traj.shape[0]), cmap=plt.get_cmap('gnuplot'),alpha = 0.7)

            axs.set_title("CPvGTI: Path " + str(i), fontsize = 25)
            ...
            if not axis:
                axs[i].axis("off")
            axs[i].scatter(adata.obsm[basis][:,0],adata.obsm[basis][:,1], color = 'black', alpha = 0.1)
            pseudo_visual = axs[i].scatter(X_traj[:,0],X_traj[:,1],c = np.arange(X_traj.shape[0]), cmap=plt.get_cmap('gnuplot'),alpha = 0.7)
            
            axs[i].set_title("CPvGTI: Path " + str(i), fontsize = 25)
...
def cp_traj_visual(CPvGTI_obj, basis = "pca", trajs = 4, 
                     figsize = (20,10), save_as = None, title = None, **kwargs):
    """\
    Description        
        cp-level trajectory visualization
    ...
    if "X_" + basis not in CPvGTI_obj.adata.obsm:
        raise ValueError("please calculate " + basis + " first")
    else:
        X_cell = CPvGTI_obj.adata.obsm["X_"+basis].copy()
        groups = CPvGTI_obj.groups
    ...
    # draw clusters 
    ax.scatter(X_clust[0:-1:1,0], X_clust[0:-1:1,1], color = 'black', alpha = 0.5)

    # draw cp trajectories
    for i in range(trajs):
        start_point = True
        for index in CPvGTI_obj.paths[CPvGTI_obj.greedy_order[i]]:
            if start_point:
...
def traj_visual(CPvGTI_obj_obj, trajs = 4, figsize = (15,10), save_as = None, title = None, axis = True, cmap = "tab20b"):
    """\
    Description        
